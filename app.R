...
eval(parse("text.R", encoding="UTF-8"))
...
# will corrupt the data because the program cant handle special characters from 
# German and Danish.

DD <- read_delim("data_file_5_post_fix_of_cause_of_death_csv.csv", 
...
#lat_long5 <- read_delim("Lat_Long.RData",delim = ";", escape_double = FALSE, trim_ws = TRUE)

lat_long5 <- readxl::read_xlsx("lat_long_1 - main.xlsx")
...
createMyLeaflet <- function (soldiers) {
  mainmap5 <-leaflet(lat_long5) %>% 
    addTiles() %>% 
    addCircleMarkers(lng = soldiers$longitude, 
                     lat = soldiers$latitude,
                     popup = paste(
                       "<strong>Navn: </strong>",soldiers$Name_of_deceased,
                       "<br><strong>Efternavn: </strong>",soldiers$Last_name_of_deceased,
                       "<br><strong>Sted: </strong>",soldiers$Place_of_death,
                       "<br><strong>Alder ved død: </strong>",soldiers$Age_of_death,
                       "<br><strong>Dato : </strong>",soldiers$Date_of_death,
                       "<br><strong>Årsag til død : </strong>",soldiers$Cause_of_death,
                       "<br><strong>Rank: </strong>",soldiers$Military_rang,
                       "<br><strong>Regiment: </strong>",soldiers$Military_Regiment_NR_At_Death,
                       "<br><strong>Fødselssted: </strong>",soldiers$Place_of_origen,
                       "<br><strong>Kommentar fra indtasteren: </strong>",soldiers$typers_coments),
                     color = "red",
                     weight = 10,
                     radius = 8,
...
ui <- fluidPage(
  titlePanel("Danskere i tysk tjeneste under WW1 som døde"),
  dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        menuItem("Liste over manglende steder", tabName = "Liste_over_manglende_steder"),
        menuItem("Forskellige Værktøjer", tabName = "forskellige_værktøjer"),
    ...
    dashboardBody(
      tabItems(
        tabItem("Liste_over_manglende_steder", div(p("Liste over steder som jeg ikke kan finde manuelt"),
                                                 fluidRow(verbatimTextOutput("LOSID")))),
        ...
        tabItem("Overvejelser_og_Konklusion", div(p("Mine overvejelser og min konklusion"),
                                                  fluidRow(box(CC),
                                                           column(width = 12)))),
        tabItem("forskellige_værktøjer", div(p("De forskellige værktøjer i programmet"),
                                                  fluidRow(box(PP),
                                                           column(width = 12)))),
        tabItem("Link_til_original_data",     div(p("Stor tak til Sønderjyske Museum og alle de frivilige ved denstorekrig1914-1918.dk"),
                                                    fluidRow(box(JJ), column(width = 12) ))),
        tabItem("Kontakt_info", div(p("Email: interaktivekort@gmail.com og link til min GitHub: https://github.com/ErikOehlerich/WW1-map-Shinyapp",
                                      fluidRow(box(KK), column(width = 12) )
                                      ))),
        tabItem("Projektet", div(p("Kort om projektet og mig"),
                                 fluidRow(box(tt),
                                          column(width = 12)))),
        tabItem("map1", leafletOutput("mainmap5"),
                fluidRow(
                  box(
                    column(width = 12),
                    checkboxGroupInput("box1", h3("Dødsårsag"),
                  ...
                  fluidRow(
                    box(
                      column(12,
                             checkboxGroupInput("box2", h3("Alder ved død"),
                                                choices = list("16" = "16",
                                                               "17" = "17",
                                                               "18" = "18",
                                                               "19" = "19",
...
#save_html(shinyApp(ui, server),"worldwar1.html") 
