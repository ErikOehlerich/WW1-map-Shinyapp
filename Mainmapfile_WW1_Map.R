...
DD <- read_delim("Rstudio_files/clean_dataset_ww1_danes.csv", 
  ...
  dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        menuItem("Beskrivelse af udfordringer", tabName = "Beskrivelse af udfordringer"),
        menuItem("Hvordan jeg behandlede dataen", tabName = "Hvordan jeg behandlede dataen"),
        menuItem("Overvejelser og Konklusion", tabName = "Overvejelser og Konklusion"),
        menuItem("Link til original data", tabName = "Link til original data"),
        menuItem("Kontakt info", tabName = "Kontakt info")
      )),
    dashboardBody(
      tabItems(
        tabItem("map"),
        tabItem("Projektet"),
        tabItem("Beskrivelse af udfordringer"),
        tabItem("Hvordan jeg behandlede dataen"),
        tabItem("Overvejelser og Konklusion"),
        tabItem("Link til original data"),
        tabItem("Kontakt info")),
      leafletOutput("mainmap5"),
      fluidRow(
        box(
          column(width = 12),
          checkboxGroupInput("box1", h3("Cause of death"),
                             choices = list("Faldet" = "Faldet", 
                                            "Kvaestelser" = "Kvaestelser",
                                            "Savnet" = "Savnet",
                                            "Sygdom" = "Sygdom",
                                            "Forlist" = "Forlist",
                                            "Verschyttet" = "Verschyttet",
                                            "Ulykke" = "Ulykke",
                                            "Druknet" = "Druknet",
                                            "Gasforgiftning" = "Gasforgiftning",
                                            "Selvmord" = "Selvmord",
                                            "Fejlagtigt meldt doed i DBR" = "Fejlagtigt meldt doed i DBR",
                                            "Fundet doed" = "Fundet doed",
                                            "Flystyrt" = "Flystyrt",
...
server <- function(input, output, session) {
  ...
  #   output$mainmap5 <-renderLeaflet({createMyLeaflet(map_data_react)})
  # output$mainmap5 <- renderLeaflet({
  #   createMyLeaflet(soldiers %>%dplyr::filter( if (is.null(input$box) & is.null(input$box2))
  
  #     { TRUE } else { Cause_of_death %in% input$box & Age_of_death %in% input$box2}
  #     ))})
}
